...
from action_gateway import ActionGateway, ToolSpec, digest_arguments  # noqa: E402
...
RUN = RunContract.from_dict({
    ...
    "deadline_at": 2_000, "scope_snapshot": ["workspace:read"], "trace_id": "trace-dispatch-001",
})
...
class GovernedDispatchTests(unittest.TestCase):
    ...
    def _loop(self, plan, dispatcher):
        ...
        return AgentLoop(RUN, self.evidence, actor_id=ACTOR, workspace_id=WORKSPACE,
                         actions={"repo.read": dispatcher.bind("repo.read")}, observer=observer)

    def _events(self):
        return [json.loads(line) for line in self.evidence.read_text().splitlines()]

    def _run(self, *, token=None, token_factory=None, gateway=None, now=1_001, **plan_kwargs):
        from governed_dispatch import GovernedActionDispatcher

        plan = AgentPlan.from_dict(plan_value(**plan_kwargs))
        provider = token_factory or (lambda: token if token is not None else authorization_token())
