...
class _ContextCaller:
    ...
    def __init__(self, written: _Written):
    ...
    def __call__(self, *, goal, context, repair_error, attempt):
        ...
        steps = [
            ...
            step(
                "write-1",
                WRITE_ACTION,
                {"path": self.written.path, "content": self.written.content},
...
class LiveRunWiringTest(unittest.TestCase):
    ...
    def run_live(self, fixture_name: str, written: _Written) -> tuple[int, dict]:
        fixture_path = SHADOW / fixture_name
        report_path = self.root / "report.json"
        stdout = io.StringIO()
        with patch.object(
            live_run, "OpenAICompatiblePlannerCaller", lambda config: _ContextCaller(written)
    ...
    def test_fixture_without_shadow_contract_leaves_the_report_unchanged(self):
        ...
        fixture_path = self.root / "plain.json"
        fixture_path.write_text(json.dumps(fixture), encoding="utf-8")
        report_path = self.root / "plain-report.json"
        with patch.object(
            live_run,
            "OpenAICompatiblePlannerCaller",
            lambda config: _ContextCaller(_Written("out/report.md", COMPLIANT)),
            ...
            code = live_run.main(
                [
                    ...
                    "--key-env", "TEST_PLANNER_API_KEY",
                    "--model", "fixture/model",
                    "--sandbox", str(self.root / "plain-sandbox"),
                    "--report", str(report_path),
                ]
            )
