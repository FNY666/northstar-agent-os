...
class RealSubprocessTests(unittest.TestCase):
    ...
    def test_a_hanging_hook_is_killed_and_denies(self):
        ...
        # Bound relaxed from 2s: on iSH, spawning the interpreter costs ~3s, so
        # the wall clock is dominated by spawn+teardown, not by the deadline. The
        # claim under test is "bounded well below the 5s sleep", which 4.5s still
        # proves; the decision/reason assertions above are the primary evidence.
        self.assertLess(elapsed, 4.5, f"the timeout did not bound the hook: {elapsed:.1f}s")
