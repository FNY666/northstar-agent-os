...
- **`autoApprove` 不是配置项，是被拒的申请**：仓库里的文件不能替操作者把撤回的审批买回来，所以非空的
  ...
  > **（2026-09-09 同日·执行边界第五批更正：括号里那句作废。）**「裁剪进程能看什么归宿主 OS」对
  > *你选定的*进程成立，对*仓库指定的*进程不成立——那条路径当时是全量继承父环境（执行边界审计的 F5）。
  > 现在：子进程环境是白名单（`PATH`/`LANG`/`LC_ALL` + 该服务器自己的 `env`），`${VAR}` 只对 `--mcp-env`
  > 点名的变量可读，其余是配置错误；起进程另需 `--mcp-allow-exec`。对账见 audit 文档 §12。
...
`mcp list` 只描述不启动（第五批起连 `${VAR}` 都不解析），`--mcp-config` 只在命令行里被打开，而打开之后
**还要** `--mcp-allow-exec` 才起进程；导入的 `env` 是便利而不是隔离——第五批之后它同时**是**一道白名单：
不在里面的父环境变量，子进程看不到。
