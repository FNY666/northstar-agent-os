...
- `components/northstar-agent-runtime/` — a governed agent loop with events, ten lifecycle hooks, a three-layer permission gate, turn/tool-call/USD ceilings, subagents, append-only sessions with optional per-session hash/HMAC integrity verification, read-only transcript replay and cross-process recovery, path-level workspace-change receipts, bounded content-addressed checkpoints with inspect/diff/rewind/fork, safe-boundary compaction, span tracing, and an experimental host-controlled local app-server for authenticated background runs. It holds no model credentials for Codex and never spawns a model CLI: Codex execution is delegated to the sidecar over its Unix socket, so reasoning and policy stay in the runtime while execution and sandboxing stay in the sidecar.
...
(1,040 tests at the current snapshot, 1,036 pass; four optional OpenTelemetry checks
