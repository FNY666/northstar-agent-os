...
assert r['property_sweep']['dimensions'] == 19
assert r['property_sweep']['combinations'] == 524288
assert sum(r['property_sweep']['counts'].values()) == 524288
assert set(r['status_distribution']) == {'RECOVERED', 'UNKNOWN', 'REJECT'}
assert all(v >= 0 for v in r['status_distribution'].values())
assert len(r['property_sweep']['single_gate_minimizer']) == 3
print('PASS: validator cases=%d sweep=524288 mutually-exclusive=RECOVERED/UNKNOWN/REJECT' % r['case_count'])
