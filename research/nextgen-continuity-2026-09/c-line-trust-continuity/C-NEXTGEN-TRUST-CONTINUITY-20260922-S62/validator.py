...
assert r['property_sweep']['dimensions'] == 24
assert r['property_sweep']['combinations'] == 16777216
assert sum(r['property_sweep']['counts'].values()) == 16777216
assert set(r['status_distribution']) == {'RECOVERED', 'UNKNOWN', 'REJECT'}
assert len(r['property_sweep']['single_gate_minimizer']) == 3
print('PASS: validator cases=%d sweep=16777216 mutually-exclusive=RECOVERED/UNKNOWN/REJECT' % r['case_count'])
